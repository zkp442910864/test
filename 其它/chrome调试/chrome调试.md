...
```
    performance 面板
        https://juejin.cn/post/6844904166918193166
        FPS。
            每秒帧数。绿色竖线越高，FPS越高。FPS图表上的红色块表示长时间帧，很可能会出现卡顿。
            （绿色, 高度越高, 性能越好）
            （红色, 即帧数已经下降到影响用户体验的程度）
        CPU。
            CPU资源。此面积图指示消耗 CPU 资源的事件类型
            CPU包括两种状态
                充满颜色
                不充满颜色
        ...
        火焰图 （刷新检查时候）
            在火焰图上看到一到三条垂直的虚线。
            绿线 FP         首次绘制的时间。
            蓝线 DCL        DOMContentLoaded 事件。
            深绿线 FCP      第一个页面丰富的画图。
            红线 L          load 事件在火焰图中选择事件时，数据统计窗格会显示与事件相关的其他信息

        火焰图可鼠标拖动，可 W(放大)、S(缩小)、A(左移)、和 D(右移) 键调整选择。
```
